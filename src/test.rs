...
pub fn verify_certificate(env: Env, cert_hash: Symbol) -> bool {
    // We clone cert_hash here because we need to use it twice: 
    // once for the key and once for the event.
    let key = DataKey::Cert(cert_hash.clone());
    
    let exists = env.storage().persistent().has(&key);
    
    // IMPORTANT: Note the '!' in symbol_short!() 
    // The topics must be in a tuple: (Topic1, Topic2)
    env.events().publish(
        (symbol_short!("verify"), cert_hash), 
        exists
    );
